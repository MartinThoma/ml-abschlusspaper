...
segmentation~\cite{Tarel2009}). The KITTI Road Estimation dataset is the basis
of our experiments (see \cref{sec:datasets}). \\
To be able to evaluate different approaches we used only the
training data of the KITTI dataset, as the ground truth for the test data is
...
\begin{table}[]
    \begin{center}
        ...
        with different strides $s$ on our own test set (58~images, $ ~6.7 \cdot
        10^6$ pixels). The table entries highlighted in red are the best
        results of their column.}
...
offical KITTI evaluation. The table also shows the values of true positive
...
\begin{table*}[]
    ...
    \begin{tabular}{c|cccccc}
        ...
        \end{tabular}
    \end{center}
    \caption{Results of the offical KITTI evaluation. (AP = Average Precision, PRE = Precision, REC = Recall, FPR = False Positive Rate, FNR = False Negative Rate)}
    \label{tab:kitti}
\end{table*}
...
\begin{enumerate}
    ...
    \item Specialisation of our two models on images with half the original
